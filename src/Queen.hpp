...
// Queen class
template <typename T>
class Queen : public Piece<T> {
public:
	Queen(bool colour, const short int* tileSize, sf::Vector2f boardPos, short int startx, short int starty);
	
	virtual std::vector<sf::Vector2i> getValidSquares(T * board);
};

template <typename T>
Queen<T>::Queen(bool colour, const short int* tileSize, sf::Vector2f boardPos, short int startx, short int starty) : Piece<T>(colour, QUEEN, tileSize, boardPos, startx, starty) {}

template <typename T>
std::vector<sf::Vector2i> Queen<T>::getValidSquares(T * board) {
