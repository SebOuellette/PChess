...
// Bishop class
template <typename T>
class Bishop : public Piece<T> {
public:
	Bishop(bool colour, const short int* tileSize, sf::Vector2f boardPos, short int startx, short int starty);
...
// Constructor
template <typename T>
Bishop<T>::Bishop(bool colour, const short int* tileSize, sf::Vector2f boardPos, short int startx, short int starty) : Piece<T>(colour, BISHOP, tileSize, boardPos, startx, starty) {}

// Valid squares
template <typename T>
std::vector<sf::Vector2i> Bishop<T>::getValidSquares(T * board) {
