...
// Pawn class
template <typename T>
class Pawn : public Piece<T> {
public:
	Pawn(bool colour, const short int* tileSize, sf::Vector2f boardPos, short int startx, short int starty);
...
// Methods
template <typename T>
Pawn<T>::Pawn(bool colour, const short int* tileSize, sf::Vector2f boardPos, short int startx, short int starty) : Piece<T>(colour, PAWN, tileSize, boardPos, startx, starty) {}

template <typename T>
std::vector<sf::Vector2i> Pawn<T>::getValidSquares(T * board) {
